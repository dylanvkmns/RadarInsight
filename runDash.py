...
def get_all_stats():
    return [
        ...
        {'label': 'pdS', 'value': 'pdS'},
        {'label': 'pdM', 'value': 'pdM'},
        {'label': 'pdPS', 'value': 'pdPS'},
        {'label': 'pdPM', 'value': 'pdPM'},
        {'label': 'Range Bias', 'value': 'Range_Bias'},  # Assuming you have columns named 'Bias1', 'Bias2', etc.
        {'label': 'Azimuth Bias', 'value': 'Azimuth_Bias'},
        {'label': 'Time Bias', 'value': 'Time_Bias'},
...
html.H1("Radar Statistics", style={'textAlign': 'center', 'paddingTop': '20px', 'color': '#34495E'}),
...
def update_bias_figure(selected_radar):
    conn = sqlite3.connect("rqmData.db")
    cursor = conn.cursor()
    cursor.execute(f"SELECT * FROM biases WHERE Radar_Name=?", (selected_radar,))
    ...
    labels = [desc[0] for desc in cursor.description][2:-1]  # Exclude Radar_Name, Antenna_Type and Job_Date
...
def update_probability_figure(selected_radar):
    conn = sqlite3.connect("rqmData.db")
    cursor = conn.cursor()
    cursor.execute(f"SELECT * FROM detection_rates WHERE ds_name=?", (selected_radar,))
    data = cursor.fetchall()
    conn.close()
    ...
    labels = [desc[0] for desc in cursor.description][2:-1]  # Exclude ds_name, ds_type, and Job_Date
    dates = [entry[-1] for entry in data]
...
def update_comparison_figure(selected_stat):
    ...
    cursor.execute(f"SELECT {radar_column}, {selected_stat}, Job_Date FROM {table_name} ORDER BY Job_Date")
    data = cursor.fetchall()
    conn.close()
    ...
    return {
        'data': [go.Scatter(x=dates, y=radar_data[radar], mode='lines+markers', name=radar) for radar in radar_names],
